...
def string_to_matrix(s, columns):
    matrix = []
    # Calculate the total rows required
    rows = (len(s) + columns - 1) // columns
    
    # Loop through and fill the matrix row by row
    for row in range(rows):
        start_index = row * columns
        end_index = start_index + columns
        matrix.append(list(s[start_index:end_index]))
        
        # If the last row is shorter, pad with empty strings
        if len(matrix[-1]) < columns:
            matrix[-1].extend([''] * (columns - len(matrix[-1])))
    
    return matrix

def whatifUJustRotateK4(ciphertext):
    ciphertext = "?" + ciphertext
    divisors = get_divisors(len(ciphertext))

    for divisor in divisors:
        ciphertextMatrix = string_to_matrix(ciphertext, divisor)
        ciphertextMatrix = rotateMatrixAnticlockwise(ciphertextMatrix)
        for divisor2 in divisors:
            ciphertextMatrix = transformMatrix(ciphertextMatrix, divisor2)
            plaintext = rotateMatrixAnticlockwise(ciphertextMatrix)
